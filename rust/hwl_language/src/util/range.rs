...
macro_rules! impl_open {
    ($R:ident) => {
        impl<T> $R<T> {
            ...
            fn bounds(self) -> (RangeBound<T>, RangeBound<T>) {
...
impl<T: Display> Debug for ClosedMultiRange<T> {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let ClosedMultiRange {
            inner: MultiRange { ranges },
        } = self;
        write!(f, "ClosedMultiRange({})", ranges.iter().format(", "))
    }
}

#[cfg(test)]
mod test {
    use crate::util::range::{MultiRange, NonEmptyRange, Range};

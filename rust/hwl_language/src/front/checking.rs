...
use crate::front::value::{DomainSignal, Value, ValueDomain};
...
impl CompileState<'_, '_> {
    ...
    pub fn check_domain_crossing(
        ...
        let invalid_reason = match (target, source) {
            ...
            (ValueDomain::Sync(target), ValueDomain::Sync(source)) => {
                ...
                match sync_domains_equal(target, source).map_err(|e| e) {
                    Ok(SyncDomainsEqual::Equal) => None,
                    Ok(SyncDomainsEqual::NotEqual(reason)) => Some(reason),
                    // pretend they match to supress downstream error
                    Err(_) => None,
                }
            }
    ...
    // TODO port the remainder of this to evaluate_expression
...
pub fn sync_domains_equal(left: &SyncDomain<DomainSignal>, right: &SyncDomain<DomainSignal>) -> Result<SyncDomainsEqual, ErrorGuaranteed> {
    let SyncDomain { clock: target_clock, reset: target_reset } = left;
    let SyncDomain { clock: source_clock, reset: source_reset } = right;

    let signal_eq = |a: &DomainSignal, b: &DomainSignal| {
        match (a, b) {
            // propagate error
            (&DomainSignal::Error(e), _) | (_, &DomainSignal::Error(e)) => Err(e),
            // equality is a valid operation for signals
            (a, b) => Ok(a == b),
        }
    };

    // we're intentionally not emitting any error if _either_ of them is an error already, to prevent confusion
    match (signal_eq(target_clock, source_clock)?, signal_eq(target_reset, source_reset)?) {

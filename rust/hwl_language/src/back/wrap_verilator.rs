...
use hwl_common::mid::ir::{IrModule, IrModules, IrPort, IrPortInfo, PortDirection};
use hwl_common::pos::Spanned;
use hwl_common::util::arena::{Arena, IndexType};
use hwl_common::util::big_int::BigUint;
...
impl VerilatedLib {
    ...
    /// * the verilated verilog generated by [hwl_common::back::lower_verilog::lower_to_verilog]
...
impl VerilatedInstance {
    ...
    pub fn get_port(&self, elab: &ElaborationArenas, port: IrPort) -> Result<CompileValue, VerilatorError> {
        ...
        let value = HardwareType::from(&port_info.ty)
            .value_from_bits(elab, &bits)
            .map_err(|e| match e {
    ...
    pub fn set_port(
        ...
        let port_ty_hw = HardwareType::from(&port_info.ty);

        let reason = TypeContainsReason::Assignment {
            span_target: port_info.debug_span,
            span_target_ty: port_info.debug_info_ty.span,
        };
        check_type_contains_value(diags, elab, reason, &port_ty_hw.as_type(), value).map_err(Either::Right)?;

        let bits = port_ty_hw
            .value_to_bits(elab, value.inner)

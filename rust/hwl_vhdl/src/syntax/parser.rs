...
pub struct FileContentRecovery {
    pub recovered_content: DesignFile,
    pub errors: Vec<ParseError>,
}

pub fn parse_file_content_without_recovery(file: FileId, src: &str) -> Result<DesignFile, ParseError> {
...
pub fn parse_file_content_with_recovery(file: FileId, src: &str) -> Result<FileContentRecovery, ParseError> {
    ...
    // actual parsing
    let result = grammar::DesignFileParser::new().parse(&mut ctx, tokenizer);

    match result {
        Ok(content) => Ok(FileContentRecovery {

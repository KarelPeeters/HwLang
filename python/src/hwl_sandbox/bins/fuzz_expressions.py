...
def fuzz_step(build_dir: Path, sample_count: int):
    ...
    compiled = compile_expression(ty_a=ty_a, ty_b=ty_b, ty_res=ty_res, expr=expression, build_dir=build_dir)
    ...
    for _ in range(sample_count):
        ...
        assert res_func == res_mod, f"Mismatch for types `{ty_a}`, `{ty_b}`, `{ty_res}`, expression `{expression}`, values `{val_a}` `{val_b}`: func {res_func} != mod {res_mod}"
